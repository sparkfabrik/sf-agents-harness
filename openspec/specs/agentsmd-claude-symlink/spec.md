# agentsmd-claude-symlink Specification

## Purpose

Define how the `sf-create-agentsmd` skill manages a `CLAUDE.md` alias next to `AGENTS.md` so that Claude Code (which reads `CLAUDE.md`) and agents.md-aware tools (which read `AGENTS.md`) share a single source of truth.
