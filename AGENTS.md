...
The `.claude/`, `.github/prompts/`, `.github/skills/`, and `.opencode/` trees hold
OpenSpec commands and skills generated by the `openspec` CLI. See
[OpenSpec Generated Files](#openspec-generated-files).
...
## OpenSpec Generated Files

This repository uses OpenSpec for its own spec-driven workflow. The commands and
skills under `.claude/`, `.github/prompts/`, `.github/skills/`, and `.opencode/`
are generated by the `openspec` CLI, not hand-written. Editing them directly is
pointless: the next regeneration overwrites the changes.

Their content depends on the global OpenSpec profile
(`~/.config/openspec/config.json`), which decides the workflow list and whether
commands, skills, or both are written. That profile is pinned in
`config/openspec-profile.json`, and `scripts/openspec-update.sh` applies it:
...
The script merges the pinned keys into the global config instead of replacing it,
so personal settings it does not pin are preserved. It always runs the latest
published CLI through `npx`, so the locally installed `openspec` version does not
affect the output. Requires `jq` and `npx`.

Two workflows keep the trees honest:

- `.github/workflows/test-openspec-update.yml` fails a pull request whose generated
  files do not match the current CLI output.
- `.github/workflows/openspec-update.yml` runs every Monday, regenerates, and opens a
  PR when anything changed. Scheduled runs squash-merge that PR automatically; manual
  dispatches leave it open for review.

Because the CLI is not pinned, a new OpenSpec release makes the pull request check
fail until someone runs the script and commits the result.
