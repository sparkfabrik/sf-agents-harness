...
- **git/url dependencies**: Review `github:`, `git+https://`, tarball URLs,
  and raw commit refs carefully
- **lifecycle scripts**: `preinstall`, `install`, `postinstall`, and `prepare`
  are executable attack surfaces
- **nested workspaces**: Audit every workspace manifest and lockfile, not just
  the repo root
- **typosquatting**: Verify package names carefully
- **Lockfile integrity**: `package-lock.json` should be committed and reviewed
- **incident triage**: For named malicious package campaigns, also read
