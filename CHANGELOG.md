...
- `glab-stack` skill (`skills/system/`): manage stacked diffs and dependent merge requests with `glab stack` -- create, save, amend, sync, and navigate a stack of small, dependent MRs. Synced from glab's bundled skill in [gitlab-org/cli](https://gitlab.com/gitlab-org/cli) with SparkFabrik notes (self-hosted host targeting, Conventional Commit messages, `:robot:` attribution on stack-opened MRs). Works on self-hosted GitLab CE -- it chains MRs by branch targeting, with no paid tier or Merge Request Dependencies API required.
- Upstream skill sync now supports GitLab-hosted upstreams via an optional `host` field in `config/upstream-skills.json` (defaults to `github.com`); `scripts/sync-skill.sh` builds GitLab archive tarball URLs when `host` is a GitLab instance.
