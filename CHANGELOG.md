...
- `provisioners/` directory with the `openspec.sh` provisioner: installs OpenSpec skills and `/opsx:*` commands globally for Claude Code by generating them from the locally installed `openspec` CLI (not vendored as static files, so they never drift from the binary), staging the workspace under `~/.cache/sparkdock/openspec` to avoid polluting `$HOME`, and symlinking the result into Claude Code's global directories. Supports `sync`/`status`/`uninstall`, is idempotent, never overwrites real user files, and exits cleanly when the CLI is absent. Also installs the `hooks/openspec-guard.sh` guard hook into `~/.claude/hooks/` and registers it in `~/.claude/settings.json` (`UserPromptSubmit` and `PreToolUse`) to block OpenSpec commands/CLI/skills outside a project that has an `openspec/` directory; registration is idempotent and preserves other hooks. `provisioners/README.md` documents the provisioner contract and the sparkdock runner hook
