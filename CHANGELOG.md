...
- `drupal-patch-convention` skill (`skills/drupal/`): apply patches to Drupal core or contrib following the SparkFabrik convention — cweagans/composer-patches with locally vendored patch files, `<NID>_<CM|MR><id>.patch` naming, `#<NID>: <title>` entry keys in `composer.patches.json`, prerequisite setup without running composer, download sanity checks, and patch removal. Extracted from the `drupal-solution-research` reference so it triggers standalone on "apply this patch" requests. Includes an eval set.

- `drupal-solution-research` skill (`skills/drupal/`): research the Drupal ecosystem for existing solutions before writing custom code, covering known core and contrib issues, patches, merge requests, alternative modules, and documented approaches. Uses the drupal.org api-d7 JSON API and the git.drupalcode.org GitLab API instead of scraping (drupal.org HTML pages are behind a JavaScript client challenge that blocks `curl`), with WebSearch for full-text discovery. Reports lead with the verdict and next action, then ranked evidence by trust level (fixed-in-release, RTBC, working patch, workaround, alternative module, custom code last). Bundles `references/drupal-org-api.md` (endpoint reference with issue status/priority/category codes and patch-file resolution), plus an eval set. Patch application delegates to the `drupal-patch-convention` skill.
